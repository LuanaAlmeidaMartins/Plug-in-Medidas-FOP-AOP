...
public class ModalController extends WizardPage {
	...
	public ModalController(Shell shell, ExtensionFile projectID2) {
		super("SPLMetrics");
		...
		StartController.aspectJLocation = null;
		StartController.jakLocation = null;
		StartController.afmLocation = null;
	}
	...
	private void aheadSelection() {
		...
		jakText.addModifyListener(e -> {
			...
			if (!source.getText().isEmpty()) {
				deco.hide();
				enableCalculate++;
				StartController.jakLocation = source.getText();
				enableFinishButton();
			} else {
				deco.show();
			}
	...
	private void aspectJSelection() {
		...
		ajText.addModifyListener(e -> {
			...
			if (!source.getText().isEmpty()) {
				deco.hide();
				enableCalculate++;
				StartController.aspectJLocation = source.getText();
				enableFinishButton();
			} else {
				deco.show();
			}
	...
	private void afmSelection() {
		...
		afmText.addModifyListener(e -> {
			...
			if (!source.getText().isEmpty()) {
				deco.hide();
				enableCalculate++;
				StartController.afmLocation = source.getText();
				enableFinishButton();
			} else {
				deco.show();
			}
