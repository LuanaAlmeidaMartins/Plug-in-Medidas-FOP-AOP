...
public class MetricsView {
	...
	public MetricsView(ArrayList<MetricsInformation> lista, Tecnologia id) {
		organizeUmaColuna(lista, id);
	}

	private ArrayList<ViewInformation> organizeUmaColuna(ArrayList<MetricsInformation> lista, Tecnologia id) {
		for (int i = 0; i < lista.size(); i++) {
			if (lista.get(i).getType().equals(Node.NON_LEAF)) {
				ViewInformation vi = new ViewInformation(lista.get(i).getFeatureName(),
						lista.get(i).getType(), organizeUmaColunaFolha(lista.get(i).getChildren(), id));
				if(Tecnologia.AFM.equals(id)) {
					vi.setMetricAfm(lista.get(i).getMetricValue());
				}
				if(Tecnologia.AJ.equals(id)) {
					vi.setMetricAspectJ(lista.get(i).getMetricValue());
				}
				if(Tecnologia.JAK.equals(id)) {
					vi.setMetricJakarta(lista.get(i).getMetricValue());
				}
				view.add(vi);
			}
		}
	...
	private ArrayList<ViewInformation> organizeUmaColunaFolha(ArrayList<MetricsInformation> lista, Tecnologia id) {
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			if (lista.get(i).getType().equals(Node.LEAF)) {
				ViewInformation vi = new ViewInformation(lista.get(i).getFeatureName(), lista.get(i).getType());
				if(Tecnologia.AFM.equals(id)) {
					vi.setMetricAfm(lista.get(i).getMetricValue());
				}
				if(Tecnologia.AJ.equals(id)) {
	...
	private void csvInfo() {
		for (int i = 0; i < view.size(); i++) {
			try (PrintWriter writer = new PrintWriter(new File(
					"C://Users//Luana//eclipse-workspace//Saida//" + "" + view.get(i).getFeatureName() + ".csv"))) {
